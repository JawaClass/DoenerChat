...
public class DummyDataLoader implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        for (int i = 1; i <= 100; i++) {
            ...
            var extras = List.of(
                MealExtra.builder().name("Extra Sauce").type(MealType.FISH).calories(1).price(BigDecimal.valueOf(0.50)).build(),
                MealExtra.builder().name("Extra Cheese").type(MealType.FISH).calories(2).price(BigDecimal.valueOf(1.00)).build()
            );
            m.addExtra(...extras);
            mainMealRepo.save(m);
        }
